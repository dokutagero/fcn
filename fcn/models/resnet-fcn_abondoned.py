...
class ResNetLayersFCN(link.Chain):
    ...
    Attributes:
        ~ResNetLayersFCN.available_layers (list of str): The list of available
    ...
    def convert_caffemodel_to_npz(cls, path_caffemodel, path_npz, n_layers=50):
        ...
        #    _transfer_resnet152(caffemodel, chainermodel)
            ...
            raise ValueError('The n_layers argument should be either (not implemented yet) 50, 101,'
                             ' or (not implemented yet) 152, but {} was given.'.format(n_layers))
...
def _make_npz(path_npz, path_caffemodel, model, n_layers):
    ...
    ResNetLayersFCN.convert_caffemodel_to_npz(path_caffemodel, path_npz, n_layers)
...
def init_from_resnet101(self, resnet101):
        for l in self.children():
            if l.name not in ['fc6']:
                l1 = getattr(resnet101, l.name)
